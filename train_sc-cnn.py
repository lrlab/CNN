...
def train(args, dataset, height, width, train_data_index, test_data_index):
    ...
    # 交差検定の始まり... 
    # 交差検定用にデータを分割
    x_train = np.array([ dataset['source'][i] for i in train_data_index ], dtype=np.float32)
    x_test = np.array([ dataset['source'][i] for i in test_data_index ], dtype=np.float32)
    y_train = np.array([ dataset['target'][i] for i in train_data_index ], dtype=np.int32)
    y_test = np.array([ dataset['target'][i] for i in test_data_index ], dtype=np.int32)
...
def main():
    ...
    k = 1
    for train_data_index, test_data_index in kfold:
        model, optimizer = train(args, dataset, height, width, train_data_index, test_data_index)
        print "========== ", i , "/ 10 =========="
